...
// ─── VIDEO FACADE (click thumbnail to load iframe) ───────────────
...
// ─── INTERNAL PAGE LINKS (home CTAs, work cards, wedding teaser) ─
document.body.addEventListener('click', e => {
  const el = e.target.closest('[data-page]');
  if (!el || el.closest('.nav-links') || el.closest('.project-card')) return;
  e.preventDefault();
  showPage(el.dataset.page);
});

// ─── SCROLL-TO LINKS (pricing "Inquire" buttons) ─────────────────
document.body.addEventListener('click', e => {
